...
js_quantify_map() ->
    ...
    iolist_to_binary(
        ["function(doc) {"
         ...
         "    const get_enabled_state = function(o) {"
         "        if (o === undefined || o.enabled === undefined) return undefined;"
         "        return o.enabled;"
         "    };"
         "    const is_call_recording_enabled = function(recording) {"
         "        if (recording) {"
         "            if ((recording.inbound && ("
         "                    get_enabled_state(recording.inbound.onnet) === true ||"
         "                    get_enabled_state(recording.inbound.offnet) === true)"
         "                ) ||"
         "                (recording.outbound && ("
         "                    get_enabled_state(recording.outbound.onnet) === true ||"
         "                    get_enabled_state(recording.outbound.offnet) === true)"
         "                )) {"
         "                    return true;"
         "            }"
         "        }"
         "        return false;"
         "    };"
         "    const is_call_recording_disabled = function(recording) {"
         "        if (recording) {"
         "            if ((recording.inbound && ("
         "                    get_enabled_state(recording.inbound.onnet) === false &&"
         "                    get_enabled_state(recording.inbound.offnet) === false)"
         "                ) &&"
         "                (recording.outbound && ("
         "                    get_enabled_state(recording.outbound.onnet) === false &&"
         "                    get_enabled_state(recording.outbound.offnet) === false)"
         "                )) {"
         "                    return true;"
         "            }"
         "        }"
         ...
         "        case 'account':"
         "            if (is_call_recording_enabled((doc.call_recording||{}).endpoint) ||"
         "                is_call_recording_enabled((doc.call_recording||{}).account)"
         "            ) {"
         "                emit(['call_recording', 'account_enabled'], 1);"
         "            } else if (is_call_recording_disabled((doc.call_recording||{}).endpoint) &&"
         "                       is_call_recording_disabled((doc.call_recording||{}).account)"
         "            ) {"
         "                emit(['call_recording', 'account_disabled'], 1);"
         "            }"
         "            break;"
         "        case 'user':"
         ...
         "                }"
         "            }"
         "            if (isArray(doc.queues) && doc.queues.length) {"
         "                emit(['agents', 'agent_' + doc.priv_level], 1);"
         "            }"
         "            if (is_call_recording_enabled(doc.call_recording)) {"
         "                emit(['call_recording', 'user_enabled'], 1);"
         "            } else if (is_call_recording_disabled(doc.call_recording)) {"
         "                emit(['call_recording', 'user_disabled'], 1);"
         "            }"
         "            break;"
         "        case 'device':"
         "            emit(['devices', doc.device_type || 'sip_device'], 1);"
         "            if (is_call_recording_enabled(doc.call_recording)) {"
         "                emit(['call_recording', 'device_enabled'], 1);"
         "            }"
         "            break;"
         "        case 'limits':"
         ...
         "            break;"
         "        case 'number':"
   	 "            if (doc.inbound_spam) emit(['advanced_inbound_services', 'spam_detection'], 1);"
         "            if (doc.inbound_cnam) emit(['advanced_inbound_services', 'smart_cnam'], 1);"
         "            if (doc.inbound_shield) emit(['advanced_inbound_services', 'shield_blacklist'], 1);"
         "            if (doc.inbound_spoof) emit(['advanced_inbound_services', 'spoofed_number_detection'], 1);"
